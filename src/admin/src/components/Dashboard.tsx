...
const Dashboard: React.FC<DashboardProps> = ({ events, onViewChange }) => {
  ...
  if (loading) return <div className="p-20 text-center font-black text-gray-300 animate-pulse uppercase tracking-[0.3em]">Vault Portal Initiating...</div>;
    ...
    <div className="space-y-8 animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          ...
          <p className="text-gray-400 font-bold text-xs uppercase tracking-widest mt-1">Real-time performance analytics</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-indigo-600 rounded-[40px] p-8 text-white relative overflow-hidden shadow-2xl shadow-indigo-200">
          <div className="flex justify-between items-start mb-6 relative z-10">
            <div className="bg-white/20 p-3 rounded-2xl backdrop-blur-md"><DollarSign size={24} className="text-white" /></div>
            <button 
                onClick={() => onViewChange('payments')}
                className="bg-white text-indigo-600 rounded-full p-2 shadow-xl hover:scale-125 transition-transform cursor-pointer border-none"
                title="Go to Ledger"
            >
          ...
          <h3 className="text-4xl font-black mt-2 relative z-10 tracking-tight">₦{totalRevenue.toLocaleString()}</h3>
          <div className="absolute -bottom-8 -right-8 w-32 h-32 bg-indigo-500 rounded-full blur-3xl opacity-40"></div>
        </div>

        <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm border-b-8 border-b-indigo-500 transition-all hover:translate-y-[-5px]">
            ...
            <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mt-1">Live Events</p>
        </div>

        <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm border-b-8 border-b-emerald-500 transition-all hover:translate-y-[-5px]">
            ...
            <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mt-1">Paid Raffle Entries</p>
        </div>
      </div>

      {/* CHART CONTAINER */}
      <div className="bg-white p-10 rounded-[50px] border border-gray-100 shadow-sm relative overflow-hidden">
          <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300 flex items-center gap-3 mb-10">
              <div className="w-2.5 h-2.5 bg-indigo-600 rounded-full"></div>
              Growth Metrics ({timeFilter})
          </h3>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
                    ...
                    <XAxis 
                        ...
                        axisLine={false} 
                        tickLine={false} 
                        tick={{fontSize: 10, fontWeight: '800', fill: '#cbd5e1', letterSpacing: '1px'}} 
                    />
                    <YAxis 
                        axisLine={false} 
                        tickLine={false} 
                    ...
                    />
                    <Bar dataKey="revenue" fill="#4f46e5" radius={[12, 12, 0, 0]} barSize={45} />
                  </BarChart>
                </ResponsiveContainer>
          </div>
      </div>
